<?php 
$songs = mysqli_query($connection, "SELECT id FROM songs ORDER BY RAND() LIMIT 107");

$playlistArray = array();

while ($row = mysqli_fetch_array($songs)){
    array_push($playlistArray, $row['id']);
}
...
$(document).ready(function() {
    ...
    var newPlaylist = <?php echo $jsonPlaylistArray; ?>;
...
function timeFromOffset(mouse, progressBar) {
    ...
    var seconds = audioElement.audio.duration * percentage / 99;
...
function setTrack(trackId, newPlaylist, play) {
    ...
    if(newPlaylist != currentPlaylist) {
        ...
        randomPlaylist = newPlaylist.slice();
        randomizeArray(randomPlaylist);
    }
